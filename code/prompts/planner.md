...
Available skills:
  ...
  computer           drive native desktop OS applications (e.g.
                     Calculator, Notepad, Excel) through a four-layer 
                     cascade (extract → deterministic → a11y → vision).
                     PREFER this when the user asks to operate a 
                     local application on their computer.
                     metadata MUST set: app_name (str, the name of the
                     application or its bundle id) and goal (str,
                     "what to do in the app").
                     Do NOT set metadata.force_path. Let the cascade
                     choose its own layer.
